...
/// Endpoint path matches XSD element name: /api/zippingworkerserviceconfiguration
...
public class ConfigurationController : ControllerBase
{
        ...
        try
        {
            lock (_configLock)
            {
                if (!System.IO.File.Exists(_configPath))
                {
                    ...
                    var defaultConfig = new ZippingWorkerServiceConfigurationType();
        ...
        [FromQuery] string? compressionlevel)
    {
        try
        {
            lock (_configLock)
            {
                ...
                {
                    var configData = new ConfigurationData(_configPath);
                    config = configData.ApplicationConfiguration ?? new ZippingWorkerServiceConfigurationType();
                    xmlDoc = configData.XmlDoc;
                }
                else
                {
                    config = new ZippingWorkerServiceConfigurationType();
    ...
    private ConfigurationResponse MapToResponse(ZippingWorkerServiceConfigurationType config)
    {
    ...
    {
        XNamespace ns = "http://tempuri.org/ZippingWorkerServiceConfigurationSchema.xsd";
        var root = new XElement(ns + "zippingworkerserviceconfiguration");
        return new XDocument(new XDeclaration("1.0", "utf-8", null), root);
    }

    private void UpdateXmlDocument(XDocument xmlDoc, ZippingWorkerServiceConfigurationType config)
    {
